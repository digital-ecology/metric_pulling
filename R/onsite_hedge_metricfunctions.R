...
pullonsitehedgeenhancement <- function(metric) {
    ...
    if (!"No Hedgerows Enhanced" %in% hedgehabitattype){
      ...
      hedgenum<-openxlsx::read.xlsx(metric, sheet = enhancementsheet, cols = 2, colNames = FALSE, startRow = 12, skipEmptyRows = TRUE)
      hedgenum <- hedgenum[hedgenum$X1 != "", , drop = FALSE]
      colnames(hedgenum) <- "hedgenumber"
      #length
      ...
      #ss
      hedgess <- openxlsx::read.xlsx(metric, sheet = enhancementsheet, cols = 21, colNames = FALSE, startRow = 12, skipEmptyRows = TRUE)
      ...
      hedgeenhancementdata<-data.frame(hedgenum, hedgehabitattype, hedgelength, hedgecond,
